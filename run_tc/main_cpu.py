# coding=utf-8
# CPU-adapted version of main.py for machines without GPU.
# Changes vs original main.py:
#   1. All `.cuda()` -> `.to(device)` so it runs on CPU.
#   2. pandas `DataFrame.append` -> `pd.concat` (append removed in pandas 2.x).
# Logic, hyper-parameters, seed and metrics are kept identical to main.py.
import torch
import random
import os
import time
...
def train_10_fold():
    ...
    for fold, (train_pos_idx, test_pos_idx) in enumerate(skf.split(df_pos_data)):
        ...
        df_train = pd.concat([df_train_pos, df_train_neg]).reset_index(drop=True).drop(columns=['Unnamed: 0'])  # 重置索引
        ...
        df_test = pd.concat([df_test_pos, df_test_neg]).reset_index(drop=True).drop(columns=['Unnamed: 0'])  # 重置索引
