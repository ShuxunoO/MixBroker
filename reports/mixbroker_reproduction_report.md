...
> ⚠️ **与 EviLink 编码器实验的"对账"**:本报告"SAGE ROC-AUC 0.86"与 EviLink"GNN AUC-PR 0.44–0.54"**看似矛盾,实则不矛盾**——指标(ROC-AUC vs AUC-PR)、图(本报告 `edge_index`=候选对,EviLink=真实 transfer 交易图)、划分(随机 KFold vs 地址不相交)都不同。**本报告的 GraphSAGE 本质是节点特征分类器(图=候选对),约等于 EviLink 的纯特征基线 R0**(同标签同折下 R0 ROC-AUC 0.887 ≳ 本报告 SAGE 0.862)。全部指标口径与对账详见 [metrics_and_cross_report_reconciliation.md](/Shuxun/AML_for_Blockchain/docs/metrics_and_cross_report_reconciliation.md)。
...
宿主机:8 核 / 16 GB,CPU-only(conda `mixbroker`,torch 2.6.0+cpu)。用 `/usr/bin/time -v` 多次实测(subset 特征表,即本报告实际使用的配置)。因宿主机被其他会话共享、负载在 load 4~88/8 核间大幅波动,故耗时以**实测区间**给出:

| 配置 | 每轮(完整 10 折)墙钟 | 每折约 | 峰值内存(Max RSS) | CPU 占用 |
|---|---|---|---|---|
| E0 基线(103 对,42d) | ~85 s | ~8~9 s | ~0.40 GB | 多线程 |
| SAGE 42d(3808/11424) | **210 ~ 395 s**(低负载端 ~210 s) | **21 ~ 40 s** | **0.43 ~ 0.44 GB** | 350% ~ 380%(≈3.5~3.8 线程) |
| SAGE 82d(3808/11424) | **130 ~ 400 s**(低负载端 ~210 s) | **13 ~ 40 s** | **0.43 ~ 0.44 GB** | 350% ~ 380% |

**说明**:
- **内存**:峰值稳定落在 **0.43 ~ 0.44 GB** 区间,远低于 16 GB;42d 与 82d 无实质差异(瓶颈是 torch 运行时与逐 epoch 评估,而非特征维度或节点表大小)。因此该实验对内存**毫无压力**。
- **耗时**:单轮开销主要来自"100 epoch × 每 epoch 在测试折上评估一次(含逐条 TP/FP 的 Python 循环 + sklearn)× 10 折",**与特征维度/节点数几乎无关**,故 42d 与 82d 落在同一区间;**无争抢时的代表值约 210 s / 轮(≈21 s / 折)**,区间上端 ~400 s 系 CPU 争抢所致。GCN/GAT 在**全量 155k 节点表**上会因强制加自环(155k 自环边)再慢 5~10×,这也是本报告改用等价的 1,484 节点裁剪表的原因(§2.4)。
- **数值稳定性**:上述墙钟区间纯由 CPU 争抢造成,**不影响任何指标数值**(种子固定 + CPU 确定性,同一配置多次运行指标逐位一致)。
